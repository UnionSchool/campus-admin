...
```bash
pnpm add @campus-admin/ui
```

通常不需要单独安装——[`@campus-admin/core`](../core) 已包含它。
...
import { CaButton, CaTable, toast } from '@campus-admin/ui'
import type { TableColumn } from '@campus-admin/ui'
...
import CampusUI from '@campus-admin/ui'
import '@campus-admin/ui/style.css'
...
<script setup lang="ts">
import { ref } from 'vue'
import { CaPagination, CaTable } from '@campus-admin/ui'
import type { TableColumn } from '@campus-admin/ui'
...
```vue
<script setup lang="ts">
import { CaSideMenu } from '@campus-admin/ui'
...
```ts
import { toast } from '@campus-admin/ui'
...
如果用 `@campus-admin/core`，推荐改用全局的 `ca.toast()`（见下方"与 ca 的关系"）。

## 与 ca 的关系

`@campus-admin/core` 提供全局命令式 API `ca`，与本库的 `toast` **共用同一份消息队列**：
...
```ts
import { setTheme } from '@campus-admin/ui'
...
```ts
import { setPrimaryColor, resetPrimaryColor } from '@campus-admin/ui'
...
组件库内置 zh-CN / en-US 两套词条，不需要额外安装 i18n 库。单独使用 `@campus-admin/ui` 时，
在入口建一个语言实例并注入应用：

```ts
import { CampusUI, createLocale, installLocale } from '@campus-admin/ui'
import { createApp } from 'vue'
...
```ts
import { setLocale, useLocale } from '@campus-admin/ui'
...
```ts
import { formatDay, formatMonth, formatNumber, weekdayLabels } from '@campus-admin/ui'
...
```ts
import { caMessages } from '@campus-admin/ui'
...
顶层 `builtInComponents` 由三层合并，`@campus-admin/core` 会自动纳入全局注册表。
...
| 包 | 关系 |
| --- | --- |
| [`@campus-admin/locale`](../locale) | 被本包依赖，组件通过它取词与格式化 |
| [`@campus-admin/core`](../core) | 依赖本包并统一出口 |
